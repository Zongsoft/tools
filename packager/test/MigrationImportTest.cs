...
public sealed class MigrationImportTest
{
	...
	public void Load_NestedImports_UsesEachSqlOriginAndItsOwnParameters()
	{
		...
		directory.Write("main.ini", "#@import child/part.ini\n[sqlite]\n./root.sql\n");
		directory.Write("child/part.ini", "#@import deep/leaf.ini\n[sqlite]\n./child.sql\n");
		...
		var plan = Loader().Load("main.ini", directory.Path, "test", "1.0.0");
	...
	{
		using var directory = new MigrationTestDirectory();
		var directive = "#@import child/part.ini\n";
		var declaration = "[sqlite]\n./schema.sql\n";
		directory.Write("main.ini", importFirst ? directive + declaration : declaration + directive);
		directory.Write("child/part.ini", declaration);
		directory.Write("main.env", "[sqlite]\nDatabase=/data/root.db\n");
		directory.Write("child/part.env", "[sqlite]\nDatabase=/data/child.db\n");
		var selected = directory.Write(importFirst ? "schema.sql" : "child/schema.sql", "SELECT 'winner';");

		var task = Assert.Single(Loader().Load("main.ini", directory.Path, "test", "1.0.0").Tasks);
	...
	{
		using var directory = new MigrationTestDirectory();
		directory.Write("main.ini", "[postgres]\n./schema.sql\n");
	...
	{
		using var directory = new MigrationTestDirectory();
		directory.Write("main.ini", "#@import child/invalid.ini\n[sqlite]\n./local.sql\n");
		var child = directory.Write("child/invalid.ini", content);
		directory.Write("sqlite.env", "Database=/data/test.db\n");
		directory.Write("local.sql", "SELECT 1;");

		var error = Assert.Throws<InvalidDataException>(() => Loader().Load("main.ini", directory.Path, "test", "1.0.0"));
	...
	{
		using var directory = new MigrationTestDirectory();
		var main = directory.Write("main.ini", "#@import child/part.ini\n[postgres]\n./root.sql\n");
		directory.Write("child/part.ini", "[postgresql]\n./child.sql\n");
	...
	{
		using var directory = new MigrationTestDirectory();
		var source = directory.Write("original/part.ini", "[sqlite]\n./schema.sql\n");
		...
		var link = Path.Combine(directory.Path, directoryLink ? "linked" : "linked.ini");
		if(directoryLink)
			Directory.CreateSymbolicLink(link, Path.GetDirectoryName(source));
		else
			File.CreateSymbolicLink(link, source);
		directory.Write("main.ini", "#@import " + (directoryLink ? "linked/part.ini" : "linked.ini") + "\n");
	...
	{
		using var directory = new MigrationTestDirectory();
		var ini = directory.Write("physical/config.data", "#@import extra.ini\n[sqlite]\n./script.sql\n");
		directory.Write("physical/extra.ini", "[unknown]\n");
		...
		File.CreateSymbolicLink(Path.Combine(directory.Path, "logical/main.ini"), ini);
		var script = Path.GetFullPath(Path.Combine(directory.Path, "logical/script.sql"));
		File.CreateSymbolicLink(script, sql);

		var tasks = Loader().Load("logical/main.ini", directory.Path, "test", "1.0.0").Tasks;
	...
	public void Load_ImportedSqlFailure_ReportsChildEntry()
	{
		using var directory = new MigrationTestDirectory();
		directory.Write("main.ini", "#@import child/part.ini\n");
		var child = directory.Write("child/part.ini", "# child\n[sqlite]\n./missing.sql\n");
	...
	public void Load_ImportCycle_FailsAndSameLoaderCanRetry()
	{
		using var directory = new MigrationTestDirectory();
		directory.Write("main.ini", "#@import child/part.ini\n");
		var child = directory.Write("child/part.ini", "#@import ../main.ini\n");
		var loader = Loader();
	...
	{
		using var directory = new MigrationTestDirectory();
		directory.Write("main.ini", "#@import missing/child.ini\n[sqlite]\n./schema.sql\n");
		directory.Write("sqlite.env", "#@import absent.env\nDatabase=/data/local.db\n");
		var sql = directory.Write("schema.sql", "SELECT 1;");
		var warnings = new List<string>();

		var task = Assert.Single(Loader(warnings.Add).Load("main.ini", directory.Path, "test", "1.0.0").Tasks);
	...
	{
		using var directory = new MigrationTestDirectory();
		directory.Write("main.ini", "#@import child/buckets.ini\n[amazon.s3]\nroot-bucket=private\n");
	...
	public void Load_DefaultImportDepth_Enforces64Files(int depth)
	{
		...
		for(var index = 0; index < depth; index++)
			directory.Write($"part-{index}.ini", index + 1 < depth ? $"#@import part-{index + 1}.ini\n" : "[sqlite]\n./schema.sql\n");
		directory.Write("sqlite.env", "Database=/data/test.db\n");
		var sql = directory.Write("schema.sql", "SELECT 'leaf';");

		if(depth == 64)
		{
			var task = Assert.Single(Loader().Load("part-0.ini", directory.Path, "test", "1.0.0").Tasks);
			Assert.Equal(sql, Assert.Single(task.Scripts).Source);
			Assert.Equal("SELECT 'leaf';", task.Scripts[0].Content);
		}
		else
		{
			var error = Assert.Throws<InvalidDataException>(() => Loader().Load("part-0.ini", directory.Path, "test", "1.0.0"));
			Assert.Contains(Path.Combine(directory.Path, "part-63.ini"), error.Message);
		}
	}
	#endregion
