...
命令行路径相对于 `--source`；INI 内的 SQL 路径相对于该 INI 文件。升迁文件及 `.env` 参数文件均使用部署工具所采用的 Zongsoft.Core `Profile` 解析器，支持整行 `#`/`;` 注释和无值条目。`Profile` 的条目名称不区分大小写，同一文件的重复键报错。同一个 INI 中重复升迁器段落（包括 PostgreSQL 两个别名并存）也会报错；不同文件和重复指定同一文件仍独立解析。
...
路径匹配由 Core Searcher 统一处理：每个模式相对固定前缀排序，Windows 匹配忽略大小写，Unix 区分大小写；文件链接保留逻辑名称并读取目标，选中的目录链接可展开，载荷内部目录链接跳过。未改变计划结构、SQL 幂等、任务顺序承诺或原生运行器。
