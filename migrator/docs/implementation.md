...
The process entry quotes individual option values and positional arguments and escapes backslashes before handing the expression to Core CommandLine. This preserves empty values and Windows paths containing spaces without merging adjacent arguments. The command writes the resolved version and Edition into its variable dictionary before calling `Normalizer.Initialize`. `--name` is independent of the version file; input/output paths use the working directory. No save operation is performed. The plan protocol and native executor do not participate in source version lookup; they receive the final identity. Command tests cover literal/file/directory/default sources, Edition selection, variable expansion and unchanged source/output files on failures.
