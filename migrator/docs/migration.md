...
| Field | Type | Meaning |
| --- | --- | --- |
| `FormatVersion` | Integer | Intermediate format version, currently `1`; distinct from application and migration tool versions. |
| `Package` | String | Normalized migration name, including the migration suffix and optional Edition, such as `zongsoft-migrate` or `zongsoft-migrate-enterprise`. |
...
| `Tasks[].Scripts[].Path` | String | Batch path relative to the migration archive extraction root, such as `.migration/.artifacts/mysql/0001.sql`. Tasks of the same provider share the directory and consecutive numbering. |
