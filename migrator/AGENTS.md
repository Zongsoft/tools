...
- 输入仅支持 .migration（Core Profile INI 格式），参数为 .ini。导入复用 Core Reader，所有来源均校验；条目路径与参数定位跟随 entry.Profile.FilePath。参数查找不隐式补齐较近配置。
...
- --version 为可选字符串：空值只读当前目录直属 .version，忽略环境和 .env 中的 version；版本号优先，否则展开文件路径，现有目录追加 .version。VersionSource 私有嵌套类复用 ApplicationVersion；Edition 按文件选择并保留拼写，name 必填且独立。解析失败含完整文件路径，不改动输出；回填最终版本和 Edition 后才初始化生成变量，输入和输出仍相对当前目录。
