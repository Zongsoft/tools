...
- `DeploymentEntry.Get` 在变量展开前取第一个冒号划分解析器。Windows 字面绝对路径需写 `:D:/...` 或用变量/相对路径；不要把盘符误当解析器。
...
- NuGet 条目是顺序文件复制，不是全图版本求解；传递依赖按其下限取得，后续条目可能降级同目录 DLL。用最终文件版本和宿主首次调用验证，不只断言复制计数。
...
Windows 命令运行需要有效控制台句柄；工具自动化使用 PTY/ConPTY。当前未定义解析器只输出错误，不累计复制失败且可能退出码为 0；断言还应检查错误输出和产物缺失。行为修复须单独获得源码修改范围，文档验证不顺带修改退出码契约。
