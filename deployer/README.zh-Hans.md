...
缺失源文件（包括包内清单引用的可选文件）和部署清单只产生警告：每项计入跳过，继续处理其余输入。默认 `.deploy`、显式清单参数及没有 `.deploy` 的目录参数均适用，方便不同环境统一传入可选文件。警告在 `--verbosity:quiet` 下也会输出，并保留在报告诊断中，不导致非零退出码。NuGet 包缺失、无效变量或语法、源链接失效、访问错误，以及计划生成后文件丢失仍作为失败处理。
